...
export default function PreacherPage() {
  ...
  const saveStats = async () => {
    ...
    if (!finalTitle.trim()) { alert("يرجى اختيار عنوان الدرس"); return; }
  ...
  const openLivePhoto = async (sessionId: string) => {
    ...
    navigator.geolocation?.getCurrentPosition(
      p => { setGpsCoords({lat:p.coords.latitude,lng:p.coords.longitude}); setGpsStatus("✅ تم تحديد الموقع"); },
      () => setGpsStatus("⚠️ لم يتم تحديد الموقع"), {enableHighAccuracy:true,timeout:10000}
    );
    try {
      ...
      <div className="container">
        ...
        {sessions.length>0&&(
          <div className="grid-3" style={{marginBottom:16}}>
            {[{label:"الجلسات",value:sessions.length,color:"#1B6B3A"},{label:"المستفيدون",value:total,color:"#C9A84C"},{label:"مكتملة",value:sessions.filter(s=>s.status==="completed").length,color:"#1D4ED8"}].map(s=>(
